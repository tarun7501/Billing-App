...
namespace Billing.Api.Controllers
{
	...
	public class BillsController : ControllerBase
	{
		...
		[HttpGet("all")]
		public async Task<IActionResult> GetBills(int pageNumber = 1, int pageSize = 10, string search = "")
		{
			var query = _context.Bills.Include(b => b.Customer).AsQueryable();
			...
			if (!string.IsNullOrWhiteSpace(search))
			{
				...
				query = query.Where(b =>
					...
					(b.Customer != null && b.Customer.PhoneNumber.Contains(search))
				);
			}
		...
		[HttpGet("generate-bill-number")]
		public async Task<ActionResult<string>> GenrateBillNumber()
		{
